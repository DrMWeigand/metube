...
class Download:
    ...
    def _download(self):
        try:
            ...
            if self.format != "subtitles":
                yt_dlp_params['progress_hooks'] = put_status
                yt_dlp_params['postprocessor_hooks'] = put_status_postprocessor
    ...
    async def update_status(self):
        while True:
            ...
            # log tmpfilename
            log.info(f'TMPFILENAME!! tmpfilename: {self.tmpfilename}')
            if 'filename' in status:
                fileName = status.get('filename')
                # log filename
                log.info(f'FILENAME!! filename: {fileName}')
    ...
    async def __download(self):
        while True:
            ...
            if entry.info.status != 'finished':
                # show tmpflename
                log.info(f'downloaded {entry.info.title} failed with status {entry.info.status}')
                log.info(f'tmpfilename: {entry.tmpfilename}')
                if entry.tmpfilename and os.path.isfile(entry.tmpfilename):
                    # print full path to tmpfilename
                    log.info(f'full path to tmpfilename: {os.path.abspath(entry.tmpfilename)}')
                    try:
                        pass
                    ...
                    log.info(f'tmpfilename: {entry.tmpfilename} not found')
